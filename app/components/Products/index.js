        ...
        <section className="text-gray-600 body-font">
            <div className="container px-5 py-24 mx-auto">
                <div className="flex flex-wrap -m-4">
                    {
                        products.map((p) => {
                            const { id, title, price, category, description, image } = p;
                            return (
                                <Link href={`/modules/${p.id}`} className="lg:w-1/4 md:w-1/2 p-4 w-full border border-opacity-50 mb-4 cursor-pointer">
